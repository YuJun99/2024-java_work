...
public class MemberFrame extends JFrame implements ActionListener{
	...
	//member.dat 파일로 부터 List<MemberDto> 객체를 얻어와서 필드에 저장하는 메소드
	...
	public void actionPerformed(ActionEvent e) {
		 ...
		 * 	따라서 문자열의 내용이 같은지 비교 할때는 동등 비교 연산자 == 를 사용하면 안 된다.
		...
		if(command.equals("add")) {
			...
			//선택된 JTableRow 가 있다면 몇번째 row 가 선택되었는지를 읽어와서
	...
	public void saveToFile() {
		...
		try {
			...
			//oos.flush(); //close() 될 때 auto flush 가 된다.
			oos.flush();
		}catch(Exception e) {
			e.printStackTrace();
		}finally {
